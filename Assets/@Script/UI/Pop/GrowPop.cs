...
public class GrowPop : UI_Pop
{
    ...
    private void Refresh()
    {
        ...
        GetText((int)Texts.Humidity_Txt).text = $"습도조건: {_data.MinHumidity}~{_data.MaxHumidity}";
        GetText((int)Texts.Weater_Txt).text = $"날씨조건: {weatherString}";
    }
    private void WaterAction(float cur, float max)
    {
        GetImage((int)Images.Water).fillAmount = cur / max;
        GetText((int)Texts.Water_Txt).text = $"습도 상태: {cur}%";
    }
